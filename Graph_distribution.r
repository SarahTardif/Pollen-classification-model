# script to create graphs with the distribution of values for the five variables that contributed the most to distinguish taxa
...
### Main figure with species for main genera ###
# load data
trainset<-read.csv('./inputs_outputs/trainset_species.csv',h=T)
...
df_plot_norm <- df_plot %>%
  mutate(value_log = log10(value + 1))  # +1 to avoid log(0)
...
### Figure for all genera ###
# load data
trainset<-read.csv('./inputs_outputs/trainset_genus.csv',h=T)
trainset$taxon<-trainset$Class
trainset$taxon<-as.factor(trainset$taxon)
trainset$Class<-NULL
